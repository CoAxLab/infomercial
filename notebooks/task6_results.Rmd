---
title: "Task 6 - BanditUniform121 - Top10 results"
...
# post-process
# Force order
result_task6$agent <- factor(result_task6$agent, levels=rev(agent_names))

# Remove E_bias
result_task6$total_E <- result_task6$total_E - E_bias
result_task6$total_E[result_task6$total_E < 0] <- 0
```
...
result_task6 %>%
  ...
  labs(x = "Time", y = "Reward", title = "Timecourse (Task 6)") +
  scale_x_continuous(limits = c(0, num_episodes + (num_episodes * .1)),
                     breaks = c(0, num_episodes)) +
