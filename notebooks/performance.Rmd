...
result_task3 %>%  
  ...
  ungroup() %>% 
  group_by(strategy, agent) %>% 
  summarise(total_R = median(total_R)) -> tmp3 
# Norm
tmp3 %>%
  group_by(agent, strategy) %>%
...
result_task4 %>%  
  ...
  ungroup() %>% 
  group_by(strategy, agent) %>% 
  summarise(total_R = median(total_R)) -> tmp4
# Norm
tmp4 %>%
  group_by(agent, strategy) %>%
...
result_task5 %>%  
  ...
  ungroup() %>% 
  group_by(strategy, agent) %>% 
  summarise(total_R = median(total_R)) -> tmp5
# Norm
tmp5 %>%
  group_by(agent, strategy) %>%
...
result_task6 %>%  
  ...
  ungroup() %>% 
  group_by(strategy, agent) %>% 
  summarise(total_R = median(total_R)) -> tmp6
# Norm
tmp6 %>%
  group_by(agent, strategy) %>%
...
result_task7 %>%  
  ...
  ungroup() %>% 
  group_by(strategy, agent) %>% 
  summarise(total_R = median(total_R)) -> tmp7
# Norm
tmp7 %>%
  group_by(agent, strategy) %>%
  mutate(total_R = total_R / max(tmp7$total_R)) ->
  tmp7
...
# -------------------------------------------------------------
# Our value
wsls_R <- filter(tmp, agent == "Curiosity (Info.)")$median_R
random_R <-  filter(tmp, agent == "Random")$median_R
