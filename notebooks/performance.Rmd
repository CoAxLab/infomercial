...
# Reference 
wsls_2 <- median((filter(tmp, strategy == "Curiosity"))$total_R)
...
tmp %>% 
  ...
  stat_summary(fun.y = median, aes(color=strategy), size=0.4, shape=1) +
...
# Reference 
wsls_3 <- median((filter(tmp, strategy == "Curiosity"))$total_R)
...
# Reference 
wsls_4 <- median((filter(tmp, strategy == "Curiosity"))$total_R)
...
# Reference 
wsls_5 <- median((filter(tmp, strategy == "Curiosity"))$total_R)
...
# Reference 
wsls_6 <- median((filter(tmp, strategy == "Curiosity"))$total_R)
...
tmp %>% 
  ...
  theme(strip.background = element_blank()) +
  scale_color_manual("", values = c("chartreuse4", "black", "darkgrey")) +
  labs(x="", y="Total reward", subtitle = "Task 5 - High dim.") +
  scale_y_continuous(limits = limits, breaks = breaks) +
  coord_flip() -> j6
...
# Reference 
wsls_7 <- median((filter(tmp, strategy == "Curiosity"))$total_R)
...
tmp %>% 
  ...
  theme(axis.text.y = element_blank()) +
  scale_color_manual("", values = c("chartreuse4", "black", "darkgrey")) +
  labs(x="", y="Total reward", subtitle = "Task 6 - Unexpected") +
  scale_y_continuous(limits = limits, breaks = breaks) +
  coord_flip() -> j7
...
# -------------------------------------------------------------
# Our value
wsls_s <- filter(tmp, strategy == "Curiosity")$median_R
...
AAAAACCCEEE###
"
perf_reg <- perf2 + j2 + j3 + j4 + j6 + j7 +
  plot_layout(design = layout, guides = "collect") +
  plot_annotation(
    title = "Reward collection performance on bandits",
...
ggsave(paste(name, ".pdf", sep=""), plot=perf_reg, width = 11, height = 5)
ggsave(paste(name, ".png", sep=""), plot=perf_reg, width = 11, height = 5)
```
# Join - deception and ditraction
```{r, fig.width=7, fig.height=3}
perf_dd <- j4 + j5 +
  plot_annotation(
    title = "Performance with deception or distraction bandits",
                  theme = theme_pubr(base_size = 14, legend = "right"),
                  tag_levels = "a", tag_suffix = ".")
print(perf_dd)
...
ggsave(paste(name, ".png", sep=""), plot=perf_dd, width = 7, height = 3)
```