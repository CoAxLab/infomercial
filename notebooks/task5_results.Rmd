---
title: "Task 5 - DistractionBanditOneHigh10 - Top10 results"
...
# Med R for curiosity
wsls_R <- median((filter(tmp, strategy == "E-explore (IG)"(IG)"))$total_R)
random_R <- median((filter(tmp, strategy == "Random"))$total_R)

tmp %>% 
  filter(strategy == "E-explore (IG)"(IG)") %>% 
  mutate(
    color = ifelse(strategy == "E-explore (IG)"(IG)", "chartreuse4", "black")) %>%
  ...
  geom_hline(aes(yintercept=random_R/num_episodes), 
             color="darkgrey",
             size=0.5) +
  labs(x="", y="", title="E-explore (IG)"(IG)") +
...
tmp %>% 
  mutate(
    fill = ifelse(strategy == "E-explore (IG)"(IG)", "chartreuse4", "black")) %>% 
...
# Med R for curiosity
wsls_R <- median((filter(tmp, strategy == "E-explore (IG)"(IG)"))$median_R)
...
# Med R for curiosity
wsls_R <- median((filter(tmp, strategy == "E-explore (IG)"(IG)"))$var_R)
...
result_task5 %>%
  ...
  labs(x = "Time", y = "Reward", title = "Timecourse (Task 5)") +
  scale_x_continuous(limits = c(0, num_episodes + (num_episodes * .1)),
                     breaks = c(0, num_episodes)) +
