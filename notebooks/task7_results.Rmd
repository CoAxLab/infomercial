...
# Med R for curiosity
wsls_R <- median((filter(tmp, strategy == "E-explore (IG)"))$total_R)
random_R <- median((filter(tmp, strategy == "Random"))$total_R)

tmp %>% 
  filter(strategy == "E-explore (IG)") %>% 
  mutate(
    color = ifelse(strategy == "E-explore (IG)", "chartreuse4", "black")) %>%
  ggplot(aes(x=agent, y=total_R, color=color)) +
  geom_boxplot(outlier.size = 0.2, width=0.5) +
  geom_hline(aes(yintercept=random_R), 
             color="darkgrey",
             size=0.5) +
  labs(x="", y="", title="E-explore (IG)") +
  # scale_y_continuous(limits = limits, breaks = breaks) +
  scale_y_continuous(limits = c(0, 7000), breaks = c(0, 7000)) +
...
tmp %>% 
  mutate(
    fill = ifelse(strategy == "E-explore (IG)", "chartreuse4", "black")) %>% 
...
# Med R for curiosity
wsls_R <- median((filter(tmp, strategy == "E-explore (IG)"))$median_R)
...
# Med R for curiosity
wsls_R <- median((filter(tmp, strategy == "E-explore (IG)"))$var_R)
...
result_task7 %>%
  ...
  labs(x = "Time", y = "Reward", title = "Timecourse (Task 7)") +
  scale_x_continuous(limits = c(0, num_episodes + (num_episodes * .1)),
                     breaks = c(0, num_episodes)) +
  facet_grid(.~agent) +
  theme_pubr(base_size = 10, legend = "none") +
