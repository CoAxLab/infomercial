---
title: "Fig. Alt. memory for E-explore"
...
alt1 <-
  (p3 / p4) + plot_annotation(title = "E-explore with alternate memory models")
print(alt1)
...
wsls_1 <- median((filter(tmp, strategy == "Ref."))$total_R)
...
tmp %>%
  ...
  geom_hline(aes(yintercept=wsls_1), 
             color="chartreuse4",
             size=0.4) +
  labs(x = "",
       y = "Total reward",
...
wsls_2 <- median((filter(tmp, strategy == "Ref."))$total_R)
...
alt1 <-
  (p3 / p4) + plot_annotation(title = "Alternate memory/distance")
print(alt1)
