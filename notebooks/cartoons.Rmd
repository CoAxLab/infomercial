...
prob_space %>% 
  ...
  theme_pubr(base_size = 5, legend = "none") +
  scale_x_continuous(limits=bools, breaks=bools) +
  scale_y_continuous(limits=bools, breaks=bools) -> p3
...
# --- build final
((p1 / p2 / p3 ) | a1 | (plot_spacer() / p4 / plot_spacer())) + 
...
points %>% 
  ...
  geom_segment(x=points$x[1], y=points$y[1], xend=points$x[3], yend=points$y[3], 
               arrow = arrow(length = unit(0.2, 'line'),type="closed"),
               size=0.3) +
  annotate("text", x = 1.5, y = 5.5, size=2, label = TeX("$f_{\\theta}(x_1)$")) +
  annotate("text", x = 6, y = 3, size=2, label = TeX("$f_{\\theta}(x_2)$")) +
  theme_pubr(base_size = 6, legend = "none") +
  theme(axis.text = element_blank()) +
  theme(axis.ticks = element_blank()) +
  theme(axis.line=element_line(arrow = arrow(length = unit(0.25, 'line')))) +
  plot_annotation(title="Learning in memory space",
                  theme = theme_pubr(base_size = 8, legend = "none"))
```
# Consistent and not - dynamics
```{r, fig.width=1.2, fig.height=1.3}
...
c_curves %>% 
  ...
  geom_vline(xintercept = t_stop, color="black", linetype="dotted", alpha=0.9, size=0.2) +
  lims(x=c(1, t_max)) + 
  labs(x="Time", y=TeX("||$\\nabla \\theta$||"), title="Consistent learning", tag="a.") +
...
ggplot(aes(x=times, y=ys, group=name)) +
  ...
  labs(x="Time", y=TeX("||$\\nabla \\theta$||"), title="Inconsistent learning", tag="b.") +
  theme_pubr(base_size = 6, legend = "none") +
  theme(axis.text = element_blank()) +
  theme(axis.ticks = element_blank()) +
...
(p1 / p2) + 
  plot_annotation(title="Learning dynamics in memory space",
                  theme = theme_pubr(base_size = 8, legend = "none"))
```
