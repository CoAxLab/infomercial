...
points %>% 
  ...
  geom_segment(x=points$x[1], y=points$y[1], xend=points$x[3], yend=points$y[3], 
               arrow = arrow(length = unit(0.2, 'line'),type="closed"),
               size=0.3) +
  annotate("text", x = 1.2, y = 5.5, size=2, label = TeX("||$f_{\\theta}(x_1)$||")) +
  annotate("text", x = 6.2, y = 3, size=2, label = TeX("||$f_{\\theta}(x_2)$||")) +
  theme_pubr(base_size = 6, legend = "none") +
  theme(axis.text = element_blank()) +
  theme(axis.ticks = element_blank()) +
...
name <- "img/cartoon2"
ggsave(paste(name, ".pdf", sep=""), plot=cartoon2, width = 1*2, height = 1*2)
```
...
name <- "img/cartoon3"
ggsave(paste(name, ".pdf", sep=""), plot=cartoon3, width = 1.2*2, height = 1.3*2)
```
