...
# Example spaces
```{r, fig.width=3.2, fig.height=3.3}
# --- An arrow
...
cartoon1 <- ((p1 / p2 / p3 ) | a1 | (plot_spacer() / p4 / plot_spacer())) + 
  plot_annotation(title="Examples of memory",
...
points %>% 
  ...
  annotate("text", x = 1.2, y = 5.5, size=3, label = TeX("$E_1$")) +
  annotate("text", x = 6.2, y = 3, size=3, label = TeX("$E_2$")) +
  theme_pubr(base_size = 8, legend = "none") +
  theme(axis.text = element_blank()) +
  theme(axis.ticks = element_blank()) +
  theme(axis.line=element_line(arrow = arrow(length = unit(0.25, 'line')))) +
  plot_annotation(title="Information value",
                  theme = theme_pubr(base_size = 10, legend = "none")) -> cartoon2
...
points %>% 
  ...
  geom_segment(x=points$x[1], y=points$y[1], xend=points$x[2], yend=points$y[2], 
               arrow = arrow(length = unit(0.2, 'line'),type="closed"),
               size=0.3) +
  annotate("text", x = 6.2, y = 3, size=3, label = TeX("$E$")) +
  theme_pubr(base_size = 8, legend = "none") +
  theme(axis.text = element_blank()) +
  theme(axis.ticks = element_blank()) +
  theme(axis.line=element_line(arrow = arrow(length = unit(0.25, 'line')))) +
...
c_curves %>% 
  ...
  lims(x=c(1, t_max)) + 
  labs(x="Time", y=TeX("$$E$$"), title="Consistent", tag="b.") +
  theme_pubr(base_size = 8, legend = "none") +
  theme(axis.text = element_blank()) +
  theme(axis.ticks = element_blank()) +
...
ggplot(aes(x=times, y=ys, group=name)) +
  ...
  labs(x="Time", y=TeX("$$E$$"), title="Inconsistent", tag="c.") +
  theme_pubr(base_size = 8, legend = "none") +
  theme(axis.text = element_blank()) +
  theme(axis.ticks = element_blank()) +
...
cartoon3 <- (p1 / p2) + 
  plot_annotation(title="Learning equilibrium",
