...
result_task1 %>% 
  ...
  mutate(
    color = ifelse(strategy == "Curiosity", "mediumpurple3", "black")) %>%
...
tmp %>% 
  mutate(
    color = ifelse(strategy == "Curiosity", "mediumpurple3", "black")) %>%
  ggplot(aes(x=agent, y=median_R, color=color)) +
  geom_point(size=1, alpha=1) +
  geom_linerange(aes(
    ymin=(median_R+var_R), 
    ymax=(median_R-var_R)), 
...
tmp %>% 
  mutate(
    color = ifelse(strategy == "Curiosity", "mediumpurple3", "black")) %>%
  ggplot(aes(x=agent, y=median_total_value, color=color)) +
  geom_point(size=1, alpha=1) +
  geom_linerange(aes(
    ymin=(median_total_value+var_total_value), 
    ymax=(median_total_value-var_total_value)), 
    alpha=0.4) +
  geom_hline(aes(yintercept=max(median_total_value)), 
             color="black",
             size=0.1) +
  labs(x="", y="Total value") +
  # lims(y=c(num_episodes/2, num_episodes)) +
...
tmp %>% 
  mutate(
    color = ifelse(strategy == "Curiosity", "mediumpurple3", "black")) %>%
  ggplot(aes(x=agent, y=median_regret, color=color)) +
  geom_point(size=1, alpha=1) +
  geom_linerange(aes(
    ymin=(median_regret+var_regret), 
    ymax=(median_regret-var_regret)), 
    alpha=0.4) +
  geom_hline(aes(yintercept=max(median_regret)), 
             color="black",
             size=0.1) +
  labs(x="", y="Total regret") +
  scale_color_identity() +
