...
## Sub 1 - policy and value
```{r, fig.height=1.4, fig.width=1.1}
...
result393 %>% 
  ...
  labs(x="", y="Information value", title="Explore") + 
  theme(strip.background = element_blank()) +
  theme_pubr(base_size = 6, legend = "none") -> p2
...
## Sub 2 - action and value
```{r, fig.height=3, fig.width=1.1}
result393 %>% 
  filter(run == r, param == p) %>% 
  filter(global_step <= n) %>%
  ...
  theme(panel.spacing = unit(0.5, "lines")) -> p1
...
result393 %>% 
  ...
  geom_line(size=0.1, color="darkorchid4") +
  geom_hline(yintercept = eta, color="darkorchid4", alpha=0.5, size=1) +
  scale_x_continuous(limits=c(-1, n+1), breaks = c(0, n/2, n)) +
...
sub2 <- p1 / (p2 + p3) + plot_layout(heights = c(0.1, 0.9))
print(sub2)
```
...
- Reward collection in a simple task, familiar to many readers.
- The task and model used often in neuroscience research.
- Need carttons

Task: BanditOneHigh4
- Explore policies are:
  - argmax(E) # det curiosity
  - softmax(R) # extrinsic
  - softmax(R + beta E) # info vsl/max
  - softmax(R + beta N) # count
  - softmax(R + novelty) # novelty


# Load task 1

Task1 - BanditOneHigh4
Ours
- meta: exp500
Random
- epsilon: exp526
...
exp_names <- c("exp457", "exp526", "exp527", "exp547", "exp502",
...
for(i in 1:length(exp_names)){
  ...
  # result_task1 <- bind_rows(result_task1, tmp1)
  summary_task1 <- bind_rows(summary_task1, tmp2)
}
...
exp_names <- c("exp408", "exp409", "exp410", "exp412", "exp514",
               "exp411", "exp513", "exp516", "exp515", "exp542")

agent_names <- c("Curiosity", "del-random", "tau-random", "Random",
                 "Extrinsic", "Info", "Novelty", "Entropy", "Count (EB)",
                 "Count (UCB)")
...
summary_task2 %>% 
  ggplot(aes(x=agent, y=total_R/max_R)) +
  # geom_jitter(width=0.1, alpha=0.6, shape=".") +
  # stat_summary(fun=mean, alpha=1, color="black", size=0.4, shape="|") +
  geom_boxplot(size=0.4, outlier.size = .1) +
  scale_y_continuous(limits=c(-.001, 0.022), breaks = c(0, 0.01, 0.02)) +
  labs(x="", y="Total reward") +
  theme_pubr(base_size = 8, legend = "none") +
  theme(strip.background = element_blank()) +
  coord_flip() -> p1
...
summary_task2 %>% 
  ...
  # stat_summary(fun=median, alpha=1, color="black", size=0.4, shape="|") +
  geom_boxplot(size=0.4, outlier.size = .1) +
  scale_y_continuous(limits=c(-.01, 0.1), breaks = c(0, 0.05, 0.1)) +
  labs(x="", y="Total regret") +
  theme_pubr(base_size = 8, legend = "none") +
  theme(strip.background = element_blank()) +
  theme(axis.text.y = element_blank()) +
...
exp_names <- c("exp403", "exp404", "exp405", "exp407", "exp510",
               "exp406", "exp509", "exp512", "exp511", "exp541")
...
for(i in 1:length(exp_names)){
  ...
  # result_task3 <- bind_rows(result_task3, tmp1)
  summary_task3 <- bind_rows(summary_task3, tmp2)
}
...
exp_names <- c("exp449", "exp450", "exp451", "exp453", "exp518",
               "exp452", "exp517", "exp520", "exp519", "exp543")
...
for(i in 1:length(exp_names)){
  ...
  # result_task4 <- bind_rows(result_task4, tmp1)
  summary_task4 <- bind_rows(summary_task4, tmp2)
}
...
exp_names <- c("exp462", "exp463", "exp464", "exp466", "exp522",
               "exp465", "exp521", "exp524", "exp523", "exp544")
...
for(i in 1:length(exp_names)){
  ...
  # result_task5 <- bind_rows(result_task5, tmp1)
  summary_task5 <- bind_rows(summary_task5, tmp2)
}
