...
result396 %>% 
  dplyr::select(
    global_step, param, run, action, total_R, 
    value_ER, total_regret, p_bests) -> tmp2

tmp1$agent <- "Deterministic"
tmp2$agent <- "Stochastic"

result393_6 <- tmp1
result393_6 <- bind_rows(result393_6, tmp2)
...
summary393_6 <- tmp1
summary393_6 <- bind_rows(summary393_6, tmp2)
```
...
## Build figure
```{r, fig.height=2.2, fig.width=1.5}
fig <- (c1 | c2 + plot_spacer() + plot_spacer()) + 
  plot_layout(widths = c(0.75, 0.25)) +
    plot_annotation(
