...
exp_names <- c(
  "exp535", "exp546", "exp429", "exp428", "exp408", 
  "exp365","exp431", "exp433")

num_episodes <- c(
  10000, 60000, 40000, 20000, 20000, 
  50000, 40000, 40000)

agent_names <- c(
  "Curiosity535", "Curiosity546", "Curiosity429", "Curiosity428", "Curiosity408",
  "Curiosity365", "tau-random431", "Random")
...
for(i in 1:length(exp_names)){
  ...
  # -
  # drop to 10000
  short_episodes <- 10000
  tmp1 %>% 
    filter(global_step <= short_episodes) -> tmp1
  tmp2 <- summarise_reward(tmp1)
  tmp2$exp <- exp_name
  tmp2$agent <- agent_name
  tmp2$task <- task_name
...
# Boxs
summary_task2_short %>% 
  ggplot(aes(x=agent, y=total_R/short_episodes)) +
  geom_boxplot(size=0.4, outlier.size = .1) +
  scale_y_continuous(limits=c(-.001, 0.02), breaks = c(0, 0.01, 0.02)) +
  labs(x="", y="Total reward") +
  theme_pubr(base_size = 8, legend = "none") +
...
summary_task2_short %>% 
  ggplot(aes(x=agent, y=total_regret/short_episodes)) +
  geom_boxplot(size=0.4, outlier.size = .1) +
  # scale_y_continuous(limits=c(-.1, 1.1), breaks = c(0, 0.5, 1)) +
  labs(x="", y="Total regret") +
  theme_pubr(base_size = 8, legend = "none") +
...
# Fig. total reward
```{r, fig.width=2.8, fig.height=1.2}
result_task2 %>% 
  filter(param <= 10, run <= 2) %>% 
  filter(global_step > 10) %>% 
  ggplot(aes(x=global_step, y=total_R/num_episodes)) +
  geom_point(size=0.1, alpha=0.6, shape=".") +
  geom_vline(xintercept = short_episodes, color="black", alpha=1, size=.1) +
