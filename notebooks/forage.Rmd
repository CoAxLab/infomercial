...
targets %>%
  ggplot(aes(x = as.integer(x_target), y = as.integer(y_target))) +
  geom_point(
    color = "black",
    shape = 4,
    size = 2,
  ...
  labs(x = "x", y = "y", subtitle = "Task 7 - Random targets") +
  theme_pubr(base_size = 9) +
  lims(x = c(-10, 10), y = c(-10, 10)) -> t1
print(t1)
```
...
results_task8 %>% 
  ...
  geom_point(
    data = targets,
    mapping = aes(x = as.integer(x_target), y = as.integer(y_target)),
    color = "black",
    shape = 4,
    size = 0.2,
    alpha = 0.4,
...
beh1 <- (t1 + e1) / (e50 + e99) +
  # plot_layout(ncol = 2) +
  plot_annotation(subtitle = "Foraging - Task, examples of behavior, and reward collection",
...
results_task8 %>%
  ...
  ggplot(aes(x = global_step,
             ...
             color = agent)) +
  geom_point(size = 1) +
  theme_pubr(base_size = 9, legend = "right") +
  labs(x = "Time", y = "Total reward", subtitle = "Reward collection timecourse during foraging") +
  scale_color_manual("Agent", 
...
# Write pdf
name <- "img/forage"
ggsave(paste(name, ".pdf", sep=""), plot=forage, width = 6, height = 8)
ggsave(paste(name, ".png", sep=""), plot=forage, width = 6, height = 8)
```
