...
tmp %>% 
  ...
  filter(strategy == "Curiosity") -> wsls_s
...
tmp %>% 
  ...
  geom_jitter(size=.2, alpha=0.01) +
  stat_summary(fun.y = median, aes(color=strategy), size=1.25, shape=1) +
...
ggsave(paste(name, ".png", sep=""), plot=consistency, width = 5, height = 5)
```
