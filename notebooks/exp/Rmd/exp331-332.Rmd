...
for (run in run_names){
  ...
  result331 <- bind_rows(result331, runtmp)  
  
}
result331$exp <- "Deterministic"
rm(runtmp, tmp)
...
for (run in run_names){
  ...
  runtmp$run <- run
  runtmp$num_trials <- nrow(tmp)
  result332 <- bind_rows(result332, runtmp)  
  
}
result332$exp <- "Stocastic"
rm(runtmp, tmp)

# --- Drop sto trials when det stopped
tmpfull <- NULL
for (n in run_names){
  tmp1 <- filter(result331, n == run)
  tmp2 <- filter(result332, n == run)
...
## Wilcox
```{r}
wilcox.test(x=log10(filter(result331, value_E > 0)$value_E), 
            y=log10(filter(result332, value_E > 0)$value_E))
```

## E dist
```{r, fig.height=0.8, fig.width=2.4}
full_join(result331, result332) %>% 
  filter(value_E > 0) %>%
...
full_join(result331, result332) %>% 
  filter(value_E > 0) %>%
  group_by(exp, run) %>% 
  summarise(total_E = sum(value_E),
            total_regret = sum(regret)) %>% 
  ungroup() ->
  tmp
tmp %>% 
 ggplot(aes(x=total_E-4*1.38, fill=exp)) +
  geom_density(alpha=0.7, color=NA) +
  scale_fill_manual("", values=c("mediumorchid4", "plum4")) +
  scale_colour_grey() +
...
full_join(result331, result332) %>% 
  ...
  tmp
tmp %>%
  ggplot(aes(x=exp, y=total_E-4*1.38, fill=exp)) +
...
tmp %>% 
  ggplot(aes(x=exp, y=total_E-4*1.38, color=exp)) +
