---
title: "Exp354 - Animals as paramters - noisy intial value only"
...
for (i in 1:length(actor_names)){
  ...
  for (run in run_codes) {
    ...
    result354 <- bind_rows(result354, runtmp)  
  }
}
...
# --
summary354$actor <- factor(summary354$actor, 
...
(c1 | c2 + plot_spacer()) + 
  plot_layout(widths = c(0.75, 0.25)) +
    plot_annotation(
      title="Initial values in a deterministic world", 
...
result354 %>% 
  ...
  theme_pubr(base_size = 8, legend = "none") +
  theme(strip.background = element_blank()) +
  scale_color_manual("Search", values=c("mediumorchid4", "dodgerblue4")) +
  scale_x_continuous(breaks=c(1, 25), labels=c(1, 25)) +
...
result354 %>% 
  ...
  labs(x="Time", y="Action", title="Deterministic + Stochastic") +
  facet_wrap(.~actor, ncol=5) +
  # scale_color_grey() +
  theme_pubr(base_size = 8, legend = "none") +
  theme(strip.background = element_blank()) +
  scale_color_manual("Search", values=c("mediumorchid4", "dodgerblue4")) +
