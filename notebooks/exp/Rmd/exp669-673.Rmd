---
title: "Analysis of exp669-673: tune runs for alternate memory models on BanditOneHigh4"
output: html_notebook
---

# Library
```{r, message=FALSE}
library(zoo)
library(ggplot2)
library(ggpubr)
library(patchwork)
library(tidyr)
library(dplyr)
library(gridExtra)
library(grid)
library(readr)
library(latex2exp)
# library(MASS) 
library(scales)
library(patchwork)
options(scipen=1000000)

# --- Main path to all data ---
data_path <- "~/Code/infomercial/data/"
```

# Load
```{r}
# What to load, and its names
exp_names <-
  c("exp673",
    "exp669",
    "exp670",
    "exp671",
    "exp672")
agent_names <- c("KL/Prob",
                 "L1/Prob",
                 "L2/Entropy",
                 "L2/Rate",
                 "UCB/Count")
colors <- c("black",
            "grey",
            "darkgrey",
            "olivedrab4",
            "orange4")
# ---
results669_673 <- NULL

for (i in 1:length(exp_names)) {
  # Meta
  exp_name <- exp_names[i]
  agent_name <- agent_names[i]
  
  # Load
  result <-
    read_csv(paste(data_path, exp_name, "_sorted.csv", sep = ""),
             show_col_types = FALSE)
  
  result$exp <- exp_name
  result$agent <- agent_name
  
  # Save
  results669_673 <- bind_rows(results669_673, result)
}

# Order
results669_673$agent <- factor(results669_673$agent, levels = agent_names)
```


# Vis
## Ranked
```{r, fig.width=6, fig.height=2}
results669_673 %>%
  ggplot(aes(x = index, y = total_R, color = agent)) +
  geom_line(size = 2, alpha = 0.6) +
  labs(x = "Rank", y = "Total reward") +
  theme_classic() +
  theme(strip.background = element_blank()) +
  theme(legend.position = "nane") +
  theme(panel.spacing = unit(1, "lines")) +
  scale_x_continuous(breaks = c(0, 500, 1000)) +
  scale_y_continuous(breaks = c(1, 100, 200), limits = c(1, 200)) +
  scale_color_manual("", values = colors) +
  facet_grid(. ~ agent) 
```

```{r, fig.width=6, fig.height=4}
results669_673 %>%
  ggplot(aes(x = index, y = total_R, color = agent)) +
  geom_line(size = 2, alpha = 0.6) +
  labs(x = "Rank", y = "Total reward") +
  theme_classic() +
  theme(strip.background = element_blank()) +
  # theme(legend.position = "nane") +
  theme(panel.spacing = unit(1, "lines")) +
  scale_x_continuous(breaks = c(0, 500, 1000)) +
  scale_color_manual("", values = colors) 
```

## Threshold
### Log
```{r, fig.width=10, fig.height=6}
results669_673 %>%
  filter(tie_threshold < 2) %>% 
  ggplot(aes(x = tie_threshold, y = total_R, color = agent)) +
  geom_point(size = 1, alpha = 0.6) +
  geom_vline(aes(xintercept=1.0), color="grey") +
  geom_vline(aes(xintercept=0.5), color="grey") +
  geom_vline(aes(xintercept=0.25), color="grey") +
  geom_vline(aes(xintercept=0.125), color="grey") +
  geom_vline(aes(xintercept=0.06), color="grey") +
  geom_vline(aes(xintercept=0.03), color="grey") +
  geom_vline(aes(xintercept=0.015), color="grey") +
  labs(x = "Boredom", y = "Total reward") +
  theme_classic() +
  theme(strip.background = element_blank()) +
  theme(legend.position = "nane") +
  theme(panel.spacing = unit(1, "lines")) +
  scale_x_log10() +
  scale_y_continuous(breaks = c(1, 100, 200), limits = c(1, 200)) +
  scale_color_manual("", values = colors) +
  facet_wrap(. ~ agent) 
```

### Raw
```{r, fig.width=10, fig.height=6}
results669_673 %>%
  filter(tie_threshold < 2) %>% 
  ggplot(aes(x = tie_threshold, y = total_R, color = agent)) +
  geom_point(size = 1, alpha = 0.6) +
  geom_vline(aes(xintercept=1.0), color="grey") +
  geom_vline(aes(xintercept=0.5), color="grey") +
  geom_vline(aes(xintercept=0.25), color="grey") +
  geom_vline(aes(xintercept=0.125), color="grey") +
  geom_vline(aes(xintercept=0.06), color="grey") +
  geom_vline(aes(xintercept=0.03), color="grey") +
  geom_vline(aes(xintercept=0.015), color="grey") +
  labs(x = "Boredom", y = "Total reward") +
  theme_classic() +
  theme(strip.background = element_blank()) +
  theme(legend.position = "nane") +
  theme(panel.spacing = unit(1, "lines")) +
  scale_y_continuous(breaks = c(1, 100, 200), limits = c(1, 200)) +
  scale_color_manual("", values = colors) +
  facet_wrap(. ~ agent) 
```

## Threshold (overlap plot)
```{r, fig.width=8, fig.height=4}
results669_673 %>%
  ggplot(aes(x = tie_threshold, y = total_R, color = agent)) +
  geom_point(size = 1, alpha = 0.6) +
  geom_vline(aes(xintercept=1), color="grey") +
  geom_hline(aes(yintercept=200/4), color="grey") +
  labs(x = "Boredom", y = "Total reward") +
  theme_classic() +
  theme(strip.background = element_blank()) +
  # theme(legend.position = "nane") +
  theme(panel.spacing = unit(1, "lines")) +
  scale_x_log10() +
  scale_y_continuous(breaks = c(1, 100, 200), limits = c(1, 200)) +
  scale_color_manual("", values = colors) 
```