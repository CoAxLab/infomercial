...
for (run in run_names){
  ...
  result333 <- bind_rows(result333, runtmp)  
  
}
result333$exp <- "Deterministic"
rm(runtmp, tmp)
...
for (run in run_names){
  ...
  runtmp$run <- run
  runtmp$num_trials <- nrow(tmp)
  result334 <- bind_rows(result334, runtmp)  
  
}
result334$exp <- "Stocastic"
rm(runtmp, tmp)

# --- Drop sto trials when det stopped
tmpfull <- NULL
for (n in run_names){
  tmp1 <- filter(result333, n == run)
  tmp2 <- filter(result334, n == run)
...
## Wilcox
```{r}
wilcox.test(x=log10(filter(result333, value_E > 0)$value_E), 
            y=log10(filter(result334, value_E > 0)$value_E))
```
## E dist
```{r, fig.height=0.8, fig.width=2.4}
full_join(result333, result334) %>% 
  filter(value_E > 0) %>%
