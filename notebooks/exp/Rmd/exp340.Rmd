...
result340$exp <- factor(result340$exp, levels=c("DeterministicActor", "SoftmaxActor", "ThresholdActor", "RandomActor"))
...
result340 %>% 
  ...
  geom_point(size=0.6) + 
  geom_line(size=0.1) +
  labs(x="Trial", y="log E") +
  facet_grid(exp~.) +
  theme_classic() + 
  scale_color_manual("", values=c("mediumorchid4", "plum4", "paleturquoise4", "grey")) +
...
print(p1)
```
# total values timecourse
```{r, fig.width=2, fig.height=0.8}
result340 %>% 
  filter(global_step > 10) %>% 
  ggplot(aes(x=global_step, y=total_E, color=exp)) +
  geom_point(size=0.6) + 
  geom_line(size=0.1) +
  labs(x="Trial", y="Total E") +
  # facet_grid(exp~.) +
  theme_classic() + 
  scale_color_manual("", values=c("mediumorchid4", "plum4", "paleturquoise4", "grey")) +
  theme(panel.spacing = unit(1, "lines")) +
...
# total reget timecourse
```{r, fig.width=2, fig.height=1}
result340 %>% 
  filter(global_step > 10) %>%
  ggplot(aes(x=global_step, y=log10(total_regret), color=exp)) +
  geom_point(size=0.6) + 
  geom_line(size=0.1) +
  labs(x="Trial", y="Total regret") +
  # facet_grid(exp~.) +
  theme_classic() + 
  scale_color_manual("", values=c("mediumorchid4", "plum4", "paleturquoise4", "grey")) +
  theme(panel.spacing = unit(1, "lines")) +
