...
result_ranked4 %>% 
  ...
  labs(x="Rank", y="Reward", title="All") +
  lims(y=c(0.2, 0.8)) +
...
result_ranked4 %>% 
  ...
  scale_color_identity() +
  labs(x="Rank", y="Reward", title="Bottom 10") +
  lims(y=c(0.2, 0.8)) +
...
# ---
supp_robust1 <- p1 + (p2 / p3 / plot_spacer() / plot_spacer()) +
  plot_annotation(title = "Reward collection and parameter choice", 
                  subtitle = "Task 6 - High dimensional",
                  tag_levels="a", tag_suffix=".")
print(supp_robust1)

name <- "img/supp_robust1"
ggsave(paste(name, ".pdf", sep=""), plot=supp_robust1, width=1.8*2, height=2.5*2)
```