...
result563 %>% 
  ...
  geom_point(size=0.6, alpha=0.9) +
  geom_line(size=0.1) +
  labs(x="Time", y="Action", title="Action" , tag="b.") +
...
# Shift scores by one, to show how E-explore works
n <- nrow(result563)
result563 %>% 
...
tmp %>%
  ...
  labs(x="", y="Info. value", title="E-explore", tag="c.") +
  scale_color_manual("", values=c("chartreuse4", "black")) +
...
independent1 <- (c1a / c1b / (c1c / c1d)) +
  ...
  plot_annotation(
      title="Explore-exploit with E-explore",
      subtitle = "Task 2",
...
filter(global_step <= num_episodes) %>%
  ...
  labs(x="Time", y="Action", title="Deterministic", tag="a.") +
...
filter(global_step <= num_episodes) %>%
  ...
  geom_point(size=0.1, alpha=0.9, shape=".") +
  geom_line(size=0.1, alpha=1) +
  labs(x="Time", y="Action", title="Stochastic (temp=0.1)") +
  lims(x=c(0, num_episodes)) +
  facet_grid(run~param) +
  scale_color_manual("", values=c("chartreuse4", "black")) +
  theme_pubr(base_size = 6, legend = "none") +
...
filter(global_step <= num_episodes) %>%
  ...
  geom_point(size=0.1, alpha=0.9, shape=".") +
  geom_line(size=0.1, alpha=1) +
  labs(x="Time", y="Action", title="Stochastic (temp=0.05)") +
  lims(x=c(0, num_episodes)) +
  facet_grid(run~param) +
  scale_color_manual("", values=c("chartreuse4", "black")) +
  theme_pubr(base_size = 6, legend = "none") +
...
independent2 <- (a1 + a2 + a3) + (s1 / plot_spacer() / plot_spacer()) + 
  plot_layout(widths = c(0.26, 0.26, 0.26, 0.22)) +
    plot_annotation(
      title="E-explore - deterministic versus stochastic reward collection",
      subtitle = "Task 2",
...
result348 %>% 
  ...
  labs(x="", y="Action", title="Deterministic", tag="a.") +
...
curiosity1 <- (sub1 | sub2) + 
  plot_layout(widths = c(0.7, 0.3)) +
    plot_annotation(
      title="E-explore: deterministic versus stochastic in information collection",
      subtitle = "Task 1",
      theme = theme_pubr(base_size=8, legend="none"))
print(curiosity1)
