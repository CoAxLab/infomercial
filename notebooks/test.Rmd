---
title: "Test analysis notebook - test1-5 and test6-9"
...
for(i in 1:length(exp_names)){
  ...
  # Join
  results_test15 <- bind_rows(results_test15, tmp)
}

# ----------------------------------------------------------------------------
# post-process
# Force order
results_test15$agent <- factor(results_test15$agent, levels=agent_names)

# Policy as factors
# Rename levels
results_test15$policy <- as.factor(results_test15$policy)
results_test15$policy <- recode_factor(results_test15$policy, `0`="explore", `1`="exploit")

# Remove E_bias
results_test15$total_E <- results_test15$total_E - E_bias
results_test15$total_E[results_test15$total_E < 0] <- 0
```
...
tmp %>% 
  ggplot(aes(x=total_R, fill=agent)) +
  geom_histogram(alpha=0.6, bins=50) +
  theme_classic() +
  labs(x="Total reward", y="Count") 
```
...
for(i in 1:length(exp_names)){
  ...
  # Join
  results_test68 <- bind_rows(results_test68, tmp)
}

# ----------------------------------------------------------------------------
# post-process
# Force order
results_test68$agent <- factor(results_test68$agent, levels=agent_names)
```
...
results_test68 %>% 
  filter(run == 5) %>% 
  ggplot(aes(x=x_pos_20, y=y_pos_20, color=agent)) +
  geom_point(alpha=0.5) +
  theme_classic() +
  labs(x="x", y="y") +
  lims(x=c(-30, 30), y=c(-30, 30))
...
results_test68 %>% 
  filter(agent != "chemotaxis") %>% 
  filter(run == 5) %>% 
  ggplot(aes(x=global_step, y=value_R_1, color=agent)) +
  geom_line(alpha=1) +
  theme_classic() +
  labs(x="Step", y="Value") 
```
...
results_test68 %>% 
  filter(agent != "chemotaxis") %>% 
  filter(run == 5) %>% 
  ggplot(aes(x=global_step, y=value_R_20, color=agent)) +
  geom_line(alpha=1) +
  theme_classic() +
  labs(x="Step", y="Value") 
```
